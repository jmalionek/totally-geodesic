...
#SBATCH --nice=10000
#SBATCH --time=7-00:00
#SBATCH --output=/data/keeling/a/chaeryn2/totally_geodesic_print/OCCcover%A_%a
#SBATCH --error=/data/keeling/a/chaeryn2/totally_geodesic_error/OCCcover%A_%a
...
if __name__ == '__main__':
    ...
    for M_index, M in enumerate(census):
        if M_index % 60 == task:
            vol = M.volume()
            low_bd = math.ceil(11.9 / vol)
            up_bd = math.floor(25 / vol)
            indices = range(max(low_bd, 2), up_bd + 1)
            for index in indices:
                for cover_index, cover in enumerate(M.covers(index)):
                    if f'totally_geodesic_info_cover{(M_index, index, cover_index)}' not in os.listdir('/data/keeling/a/chaeryn2/totally_geodesic/'):
                        gc.collect()
