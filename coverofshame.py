...
if __name__ == '__main__':
    ...
    for M_index, M in enumerate(census):
        if M_index % 60 == task:
            vol = M.volume()
            low_bd = math.ceil(7.2 / vol)
            up_bd = math.floor(25 / vol)
            indices = range(max(low_bd, 2), up_bd + 1)
            for index in indices:
                for cover_index, cover in enumerate(M.covers(index)):
                    if f'totally_geodesic_info_cover_{M_index}_{index}_{cover_index}' not in os.listdir('/data/keeling/a/chaeryn2/tg_computation_outputs/'):
                        gc.collect()
