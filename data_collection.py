...
def weekly_check():
	...
	for keyword in keywords:
		...
		print('number done:', len(results))
		print('"number with totally geodesic surfaces":', len([l for l in results['tot_geo'] if len(l) > 0]))
		print('number which ACTUALLY have tot geo', len([i for i in range(len(manifolds)) if len(results['tot_geo']) > 0 and manifolds[i].solution_type(enum=True) <= 2]))
		print()
